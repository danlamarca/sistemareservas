BEGIN TRANSACTION 
	...
	BEGIN
		CREATE TABLE TB_RESERVA(
			...
			CONSTRAINT FK_TB_RESERVA_ACOMPANHAMENTO_ITEM FOREIGN KEY(CODACOMPANHAMENTOITEM) REFERENCES TB_ACOMPANHAMENTO_ITEM
		)
	END

	IF NOT EXISTS (SELECT * FROM SYS.TABLES WHERE NAME ='TB_RESERVA_TEMP')
	BEGIN
		CREATE TABLE TB_RESERVA_TEMP(
			CODRESERVATEMP INT IDENTITY(1,1) NOT NULL,
			CODSALA INT,
			CODUSUARIO INT,
			DATAINICIO DATETIME,
			DATAFIM DATETIME,
			DESCRICAO VARCHAR(1000),
			CODACOMPANHAMENTOITEM INT
			CONSTRAINT PK_RESERVA_TEMP  PRIMARY KEY(CODRESERVATEMP), 
			CONSTRAINT FK_RESERVA_SALA_TEMP FOREIGN KEY(CODSALA) REFERENCES TB_SALA,
			CONSTRAINT FK_RESERVA_USUARIO_TEMP FOREIGN KEY(CODUSUARIO) REFERENCES TB_USUARIO,
			CONSTRAINT FK_TB_RESERVA_TEMP_ACOMPANHAMENTO_ITEM FOREIGN KEY(CODACOMPANHAMENTOITEM) REFERENCES TB_ACOMPANHAMENTO_ITEM
		)
	END
